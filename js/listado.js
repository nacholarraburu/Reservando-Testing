...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    return restaurantFiltrado ? restaurantFiltrado : "No se ha encontrado ningún restaurant";
}
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    //Se recorre el array de restaurantes y se va agregando al array arregloCiudades, todas las ubicaciones o ciudades encontradas
    var arregloCiudades = this.restaurantes.map(restaurant => restaurant.ubicacion)
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ciudadesFiltradas = arregloCiudades.filter(filtrarDatos);
...
Listado.prototype.obtenerRubros = function() {
    ...
    var arregloRubros = this.restaurantes.map(restaurant => restaurant.rubro)
    var rubrosFiltrados = arregloRubros.filter(filtrarDatos);
...
Listado.prototype.obtenerHorarios = function() {
    ...
    //En este array vamos agregando todos los array de horarios, que luego vamos convertir en un solo array
    var arregloHorarios = this.restaurantes.map(restaurant => restaurant.horarios)
    ...
    arregloHorarios.forEach(function(horario) {
        horario.forEach(function(hora) {
            horarioAgregado.push(hora)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosFiltrados = horarioAgregado.filter(filtrarDatos);
...
function filtrarDatos(elem,index,self){
